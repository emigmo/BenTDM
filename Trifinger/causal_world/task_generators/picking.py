...
class PickingTaskGenerator(BaseTask):
    def __init__(self,
                ...
                dense_reward_weights=np.array([250, 0, 125, 0, 750, 0, 0, 0.005]),
        ...
        super().__init__(task_name="picking",
                         ...
                         reference=reference,
                         mode=mode, 
                         mass=tool_block_mass, 
                         shape=tool_block_shape, 
                         size=tool_block_size)
        ...
        # random tool block position
        tool_block_position[:2] = np.random.uniform(-0.08, 0.08, size=2)
    ...
    def _set_up_stage_arena(self):
        ...
        creation_dict = {
            'name': "tool_block",
            'shape': self._task_params["tool_block_shape"],
            ...
            'size': np.array([self._task_params["tool_block_size"], 
        ...
        # TODO: observation do not use tool_block type/size and goal_block type/size
    ...
    def _set_intervention_space_a(self):
        ...
        for visual_object in self._stage.get_visual_objects():
            self._intervention_space_a[visual_object]['cylindrical_position'][0][-1] = 0.08
            self._intervention_space_a[visual_object]['cylindrical_position'][1][-1] = 0.20
        return
    ...
    def _set_intervention_space_b(self):
        ...
        for visual_object in self._stage.get_visual_objects():
            self._intervention_space_b[visual_object]['cylindrical_position'][0][-1] = 0.20
            self._intervention_space_b[visual_object]['cylindrical_position'][1][-1] = 0.25
        return
    ...
    def _calculate_dense_rewards(self, desired_goal, achieved_goal):
        ...
        rewards = list()
        block_position = self._stage.get_object_state('tool_block', 'cartesian_position')
        target_height = self._stage.get_object_state('goal_block', 'cartesian_position')[-1]
        joint_velocities = self._robot.get_latest_full_state()['velocities']
        previous_block_to_goal = abs(self.previous_object_position[2] - target_height)
        ...
        rewards.append(-np.linalg.norm(joint_velocities - self.previous_joint_velocities))
        update_task_info = {
