...
from robel.dclaw.turn import BaseDClawTurn, BaseDClawTurnTS
...
class BaseDClawScrewTS(BaseDClawTurnTS):
    """Shared logic for DClaw screw tasks."""
...
@configurable(pickleable=True)
class DClawScrewFixedTS(BaseDClawScrewTS):
    """Rotates the object with a fixed initial position and velocity."""
...
@configurable(pickleable=True)
class DClawScrewRandomTS(BaseDClawScrewTS):
...
@configurable(pickleable=True)
class DClawScrewRandomDynamicsTS(DClawScrewRandomTS):
    """Rotates the object with a random initial position and velocity.
...
@configurable(pickleable=True)
class DClawScrewRandomDamp(DClawScrewRandom):
    """Rotates the object with a random initial position and velocity.
...
@configurable(pickleable=True)
class DClawScrewRandomDampTS(DClawScrewRandomTS):
    """Rotates the object with a random initial position and velocity.
...
class DClawScrewGivenDampTS(BaseDClawScrewTS):
    """Rotates the object with a random initial position and velocity.
    ...
    def __init__(self,
                 *args,
                 sim_observation_noise: Optional[float] = 0.0,
                 **kwargs):
        super().__init__(
            *args, sim_observation_noise=sim_observation_noise, **kwargs)
        self._randomizer = SimRandomizer(self)
        self._dof_indices = (
            self.robot.get_config('object').qvel_indices.tolist())
        self.damp = 0.1  # default damp factor
        self.friction = 0.001  # default friction factor

    def given_damp(self, damp_factor: float = 0.1):
        self.damp = damp_factor # set damp factor
    
    # def given_friction(self, friction_factor: float=0.001):
    #     self.friction = friction_factor # set friction factor
    ...
    def _reset(self):
        ...
        self._randomizer.randomize_dofs(
            self._dof_indices,
            damping_range=(self.damp, self.damp),
            # friction_loss_range=(self.friction, self.friction),
        )
        super()._reset()


class DClawScrewGivenDamp(BaseDClawScrew):
    """Rotates the object with a random initial position and velocity.
...
class DClawScrewGivenDampAndFric(BaseDClawScrew):
    """Rotates the object with a random initial position and velocity.
       The dynamics of the simulation are randomized each episode.
    """
